...
<div class="container">
	...
	@foreach($galerikategoris as $galeri)
		...
		<div class="col-md-4 col-xs-12">
			<div class="card">
				<a href="{{ url('galerikategori/'.$galeri->id) }}"><img src="{{ URL::asset('files/galeri/'.$aum->id.'/'.$img['filename']) }}" alt="Thumbnail Galeri {{ $galeri->name }}" class="img-top img-fluid w-100 galeri"></a>
				<div class="card-block">
					<h5 class="card-title">{{ $galeri->name }} <small class="text-muted">{{ $galeri_c }} Gambar</small></h5>
					<a href="{{ url('galerikategori/'.$galeri->id) }}" class="btn btn-sm btn-primary">Lihat Galeri Ini</a>
				</div>
			</div>
		</div>
	@endforeach
